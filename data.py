...
class HELLO:
    def __init__(self, language='3', sleeper=10):
        ...
        config_file_path = str('configuration/configuration.cfg')
        config.read(config_file_path, encoding="utf8")
        self.mysql_ip = config.get('mysql', 'ip')
        self.mysql_user = config.get('mysql', 'user')
        self.mysql_database = config.get('mysql', 'database')
        self.mysql_table = config.get('mysql', 'table')
        self.mysql_password = ''
        self.connect=None
        self.cursor=None
    ...
    async def kick_off(self, my_loop):
        ...
        try:
            ...
            app.router.add_route('GET', '/database/{data}/{database}', self.store_to_database)
    ...
    async def store_to_database(self, request):
        connect = pymysql.connect(host = self.mysql_ip, db = self.mysql_database, user = self.mysql_user, password = self.mysql_password, charset = 'utf8', use_unicode = True)
        cursor = connect.cursor()
        data = request.match_info['data']
        database = request.match_info['database']
        if database == "Mysql":
            insertsql = "INSERT INTO "+self.mysql_table+ " (value) VALUES ('%s')" % data  #插入資料庫的SQL
